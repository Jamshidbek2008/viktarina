...
function showResult() {
    ...
    if (userScore > 3) {
        let scoreTag = '<p>🥳 Wooow siz' + userScore + ' tasiga javob beroldingiz!</p>'
        scoreText.innerHTML = scoreTag;
    }
    if (userScore > 4) {
        let scoreTag = '<p>🥳 qoyil siz' + userScore + ' tasiga javob beroldingiz!</p>'
        scoreText.innerHTML = scoreTag;
    }  else if(userScore > 1) {
        let scoreTag = '<p>😉 Yaxshi' + userScore + ' tasiga javob berolding!</p>'
        scoreText.innerHTML = scoreTag;
    } else {
        let scoreTag = '<p>😐 Afsuski sizni balingiz yetarli emas!</p>'
        scoreText.innerHTML = scoreTag;
    } if(userScore > 5) {
        let scoreTag = '<p>😉 brwo ochin priatno' + userScore + ' tasiga javob berolding!</p>'
        scoreText.innerHTML = scoreTag;
    }
}
